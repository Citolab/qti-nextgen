...
const buildOptions = {
  ...
  entryPoints: [
    ...
    './src/lib/component-editor/index.ts',
    './src/lib/component-editor-addons/index.ts'
  ],
  external: ['@lit/react', '@lit/context', 'react', 'lit', '@lit/reactive-element', '@lit', 'lit-html/src', 'haunted'], // peerdependencies
  splitting: true,
...
async function buildTS(options: Options) {
  return tsup
    ...
    .then(result => {
      console.log(result);
      console.log(chalk.green(`ce-components has been generated at ${outdir}\n`));
    });
}
